...
`sincronizarFaturas` mantém no Flow um lançamento `previsto` por fatura com total > 0:
`boxId` = box do cartão, `categoriaId` = `categoriaFaturaId`, `data` = vencimento,
`valor` = total, `status: 'previsto'`, `origem: 'cartao'`, `cartaoId`, `faturaMes`.
...
- **Dexie:** quatro tabelas novas (`cartoes`, `categoriasCartao`, `comprasCartao`,
  `recorrenciasCartao`) com bump de versão do schema; migração automática, dados
  existentes intactos.
...
- **Sincronização:** previsto atualiza ao vivo; efetivo nunca é tocado; descartado não
  ressuscita; fatura zerada remove o previsto; desativar cartão limpa previstos.
- **UI:** adicionar compra parcelada e ver as faturas corretas; confirmar fatura pendente
  pelo Hoje; tela sem cartão cadastrado; cadastro de cartão nos Ajustes.
