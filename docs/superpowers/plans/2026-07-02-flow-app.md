...
- Valores de lançamento são normalmente positivos (a direção vem do `tipo` da categoria). **Negativos são permitidos** para estornos/acertos importados da planilha (ex.: "Emprestimo B" −45,00). Validação de formulário manual exige valor > 0; o motor aceita negativo (o sinal inverte naturalmente). *(Desvio consciente da spec, que dizia "sempre positivos" — a planilha real contém células negativas e a conferência ao centavo exige importá-las como estão.)*
...
function rec(sobrescrever: Partial<Recorrencia> = {}): Recorrencia {
  return {
    id: 'r1', boxId: 'b1', categoriaId: 'c1', valor: 12500,
...
function lanc(data: string, status: 'efetivo' | 'previsto', id = `l-${data}`): Lancamento {
  return {
    id, boxId: 'b1', categoriaId: 'c1', data, valor: 12500, status,
...
it('salvarRecorrencia materializa previstos até o horizonte', async () => {
  ...
  await repo.salvarRecorrencia(
    { boxId: box.id, categoriaId: gasto.id, valor: 12500, dataInicio: '2026-01-03', diaDoMes: 3, parcelas: 8 },
    '2026-12-31',
  );
  const dados = await repo.carregarTudo();
...
it('aplicarImport é idempotente (reimportar não duplica)', async () => {
  const agora = agoraISO();
  const boxImp: Box = { id: 'bi', nome: 'eitor', saldoInicial: 100000, dataSaldoInicial: '2026-01-01', criadoEm: agora, alteradoEm: agora };
...
- Abas box (`box (pessoa-a)`, `box (pessoa-b)`, `box (casa)`): **colunas = dias, linhas = categorias**.
  ...
  - Coluna A nas linhas dos ranges: nome da categoria; linhas sem rótulo são vagas e devem ser ignoradas. Algumas células de rótulo são fórmulas (ex.: `A25 = Simulacoes!A2`) — usar o **valor** (`v`), não a fórmula.
...
- Aba `Simulacoes`: blocos de empréstimo na coluna A — um rótulo (ex.: `Emprestimo A` em A2), depois `valor total`, `data inicio` (B = serial), `dia de pagamento`, `parcelas`, `valor mensal` (pode ser negativo, ex.: Emprestimo B = −91). Localizar blocos procurando células A = `data inicio`; o nome do bloco é o primeiro rótulo não vazio acima que não seja `valor total`.
- Abas ignoradas no import: `Salarios`, `Acertos`, `Pivot`, `Resumos`, `transposed(*)`, `aux_dias`.
...
describe('lerSimulacoes', () => {
  it('extrai blocos de empréstimo', () => {
    expect(lerSimulacoes(simulacoesFixture())).toEqual([{
      nome: 'Emprestimo Teste', dataInicio: '2025-07-03', diaDoMes: 3, parcelas: 8, valorMensalCent: 12500,
    }]);
  });
});
...
describe('montarResultado', () => {
  ...
  it('empréstimo vira recorrência e vincula lançamentos da categoria homônima', () => {
    ...
    expect(rec).toMatchObject({ valor: 12500, diaDoMes: 3, parcelas: 8, origem: 'import', ativa: true });
...
4. Recorrência do Emprestimo A visível em Ajustes → Recorrências (origem import).
