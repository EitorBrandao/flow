...
- Valores de lançamento são normalmente positivos (a direção vem do `tipo` da categoria). **Negativos são permitidos** para estornos/acertos importados da planilha (ex.: "Emprestimo B" −91,00). Validação de formulário manual exige valor > 0; o motor aceita negativo (o sinal inverte naturalmente). *(Desvio consciente da spec, que dizia "sempre positivos" — a planilha real contém células negativas e a conferência ao centavo exige importá-las como estão.)*
