...
const eitor: Box = { id: 'be', nome: 'eitor', saldoInicial: 100000, dataSaldoInicial: '2026-01-01', ...ts };
...
it('acumula saldo dia a dia como a planilha', () => {
  const serie = projetarBoxes(['be'], entrada([
    ...
    lanc({ id: '2', data: '2026-01-03', valor: 12500, categoriaId: 'cd', status: 'previsto' }), // gasto previsto
  ]));
  expect(serie.map((s) => s.data)).toEqual([
    '2026-01-01', '2026-01-02', '2026-01-03', '2026-01-04', '2026-01-05',
  ]);
  expect(serie[0]).toEqual({ data: '2026-01-01', saldoEfetivo: 100000, saldoProjetado: 100000, saldoComCenarios: 100000 });
  expect(serie[1].saldoEfetivo).toBe(250000);
  expect(serie[2].saldoEfetivo).toBe(250000);      // previsto não entra no efetivo
  expect(serie[2].saldoProjetado).toBe(237500);    // 250000 - 12500
});
...
it('ignora lançamentos com data <= dataSaldoInicial (regra do dia inicial)', () => {
  ...
  expect(serie[0].saldoEfetivo).toBe(100000);
  expect(serie[4].saldoEfetivo).toBe(100000);
});

it('valor negativo inverte o sinal naturalmente (estorno)', () => {
  const serie = projetarBoxes(['be'], entrada([
    lanc({ id: '1', data: '2026-01-02', valor: -4500, categoriaId: 'cd' }), // gasto negativo = +45
  ]));
  expect(serie[1].saldoEfetivo).toBe(100000 + 4500);
});
...
it('cenário só entra em saldoComCenarios e apenas se ligado', () => {
  ...
  expect(desligado[1].saldoComCenarios).toBe(100000);
  const ligado = projetarBoxes(['be'], entrada(lancs, ['cen1']));
  expect(ligado[1].saldoProjetado).toBe(100000);
  expect(ligado[1].saldoComCenarios).toBe(70000);
});
