...
const ABAS_OBRIGATORIAS = ['box (pessoa-a)', 'box (pessoa-b)', 'box (casa)', 'Simulacoes'] as const;
...
export function lerPlanilha(dados: ArrayBuffer | Uint8Array, hoje: ISODate): ResultadoImport {
  ...
  const imps = [
    lerBoxSheet(wb.Sheets['box (pessoa-a)'], 'eitor'),
    lerBoxSheet(wb.Sheets['box (pessoa-b)'], 'ju'),
    lerBoxSheet(wb.Sheets['box (casa)'], 'casa', ['Eitor', 'Ju']),
  ];
  return montarResultado(imps, lerSimulacoes(wb.Sheets['Simulacoes']), hoje);
}
