 ...
 *
 * Só os schemas das versões 1, 2, 3, 4 e 5 são literais aqui (história congelada — nunca mude
...
/** Schema da v5 do FlowDB — literal, histórico, nunca mude. Publicado na v0.27.0: existem
 *  usuários rodando exatamente este schema, e backups carimbados com ele. */
const SCHEMA_V5 = {
  boxes: 'id',
  categorias: 'id, boxId',
  lancamentos: 'id, boxId, data, recorrenciaId, cenarioId, origem, cartaoId, viagemId',
...
/** Ajuste de fechamento introduzido na v5. */
function dadosAjuste(cartaoId: string) {
  const agora = agoraISO();
  const ajusteFechamento: AjusteFechamento = {
    id: novoId(), cartaoId, mes: '2026-08', diaFechamento: 18,
    criadoEm: agora, alteradoEm: agora,
  };
...
describe('caminho de upgrade do schema Dexie (FlowDB real)', () => {
  it('salto v1 → v6: dados de um cliente antigo sobrevivem e as tabelas novas nascem vazias', async () => {
  ...
  it('salto v2 → v6: dados de cartão sobrevivem, viagens nasce vazia e o índice viagemId funciona', async () => {
  ...
  it('salto v3 → v6: dados sobrevivem e bancos, ajustesFechamento e notasFiscais nascem vazias', async () => {
  ...
  it('salto v4 → v6: dados de banco sobrevivem e ajustesFechamento e notasFiscais nascem vazias', async () => {
    ...
    try {
      ...
      // O índice de `notasFiscais` é não-único de propósito: duas notas da mesma compra podem
      // coexistir depois de um merge de backups, e é a UI que escolhe a mais recente (ver
      // `notaDaCompra`).
      const agora = agoraISO();
  ...
  it('salto v5 → v6: os ajustes de fechamento de um cliente da v0.27.0 sobrevivem e notasFiscais nasce vazia', async () => {
    const nome = `flow-teste-v5-${novoId()}`;
    const { box, categoriaGanho, categoriaGasto, lancamentoEfetivo, lancamentoPrevisto, recorrencia, cenario, config } = dadosBase();
    const { cartao, categoriaCartao, compraCartao, recorrenciaCartao, conferenciaFatura } = dadosCartao(box.id);
    const { banco } = dadosBanco(box.id);
    const { ajusteFechamento } = dadosAjuste(cartao.id);
    ...
    antigo.version(3).stores(SCHEMA_V3);
    antigo.version(4).stores(SCHEMA_V4);
    antigo.version(5).stores(SCHEMA_V5);
    try {
      await antigo.open();
      await antigo.table('boxes').add(box);
      await antigo.table('categorias').bulkAdd([categoriaGanho, categoriaGasto]);
      ...
      await antigo.table('conferenciasFatura').add(conferenciaFatura);
      await antigo.table('bancos').add(banco);
      await antigo.table('ajustesFechamento').add(ajusteFechamento);
    } finally {
      await antigo.close();
    }
    ...
    try {
      ...
      // O ajuste gravado na v5 sobrevive, e o índice composto continua servindo a consulta.
      expect(await flow.ajustesFechamento.get(ajusteFechamento.id)).toEqual(ajusteFechamento);
      await expect(flow.ajustesFechamento.where('[cartaoId+mes]').equals([cartao.id, '2026-08']).toArray())
        .resolves.toEqual([ajusteFechamento]);

      // notasFiscais nasce na v6: vazia, e com o índice `compraCartaoId` já utilizável.
      await expect(flow.notasFiscais.count()).resolves.toBe(0);
      await expect(flow.notasFiscais.where('compraCartaoId').equals(compraCartao.id).toArray())
        .resolves.toEqual([]);
    } finally {
      await flow.close();
      await Dexie.delete(nome);
  ...
  // Guarda de versão: ao adicionar `this.version(7)` em database.ts, este teste falha de
  // propósito — é o lembrete forçado para escrever o salto 6 → 7 (schema congelado + teste
  // de migração) antes de mexer no schema real.
  it('a versão atual do schema é 6 — subiu de versão? adicione o salto novo aqui', async () => {
