...
| Dados reais | `scripts/verificar-dados-reais.mjs` | **nada ainda** — varre todos os arquivos versionados atrás de valor em real e dos termos de `~/.claude/flow-dados-reais.txt`; hoje é relatório (`--strict` sai 1, mas o release não o chama). Rode: `node scripts/verificar-dados-reais.mjs` |
