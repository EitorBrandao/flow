...
| Dados reais | `scripts/verificar-dados-reais.mjs --strict`, chamado pelo release | valor em real fora das exceções sintéticas, ou termo de `~/.claude/flow-dados-reais.txt`, em **qualquer** arquivo versionado. Arquivo de teste e dossiê gerado pulam só o padrão genérico de moeda — os termos privados valem neles também. Rode sozinho quando quiser: `node scripts/verificar-dados-reais.mjs` — sem `--strict` ele só avisa (exit 0) |
...
- **Consistência entre telas é requisito, não detalhe.** O foco do app é a consistência — dos dados e das telas. O mesmo conceito aparece do mesmo jeito em toda tela: mesmo texto, mesma cor, mesmo sinal, mesmo ponto de vista. Antes de mudar como a UI mostra um conceito, procure as outras telas que mostram o mesmo conceito. Busque pelo texto, pela classe e pela função de domínio. Se a sua mudança deixar outra tela diferente, **avise o usuário antes de implementar** e proponha alinhar as duas no mesmo branch. Nunca deixe a divergência para depois em silêncio. Exemplo: a diferença da conferência de saldo (Hoje → Conferir) e a da conferência da fatura (Cartão → Conferência) usam as mesmas frases, cores e sinal.
